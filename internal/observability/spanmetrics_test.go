...
func TestRuntime_snapshotMetrics_counts_generate_persona_as_LLM_usage(t *testing.T) {
	runtime, err := NewRuntime()
	require.NoError(t, err)
	t.Cleanup(func() {
		require.NoError(t, runtime.Shutdown(context.WithoutCancel(t.Context())))
	...
	recordLLMUsageSpan(t, "api.openrouter.generate_persona", "anthropic/claude-3-haiku", ResultTool, 30, 15, 0.5)
