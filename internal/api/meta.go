...
var personaTemplateListSchema = generateSchema[personaTemplateListWrapper]()
...
func personaListResponseFormat() openai.ChatCompletionNewParamsResponseFormatUnion {
	return openai.ChatCompletionNewParamsResponseFormatUnion{
		OfJSONSchema: &shared.ResponseFormatJSONSchemaParam{
			JSONSchema: shared.ResponseFormatJSONSchemaJSONSchemaParam{
				Name:   "persona_list",
				Schema: personaTemplateListSchema,
				Strict: openai.Bool(true),
			},
		},
	}
	...
	err := c.inSpan(ctx, "api.openrouter.generate_persona_templates",
		...
		func(ctx context.Context, span trace.Span) error {
			resp, rawResp, err := c.chatCompletion(ctx, openai.ChatCompletionNewParams{ //nolint:bodyclose // SDK reads and closes the body.
				...
				ResponseFormat: personaListResponseFormat(),
			})
			if err != nil {
				markSpanError(span, observability.ErrorKindTransport, 0, err)
