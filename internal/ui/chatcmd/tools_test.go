...
func TestBuildToolRegistry_returns_expected_tools(t *testing.T) {
	...
	require.Equal(t, []api.ToolDefinition{
		...
		{
			Name:        "pass",
			Description: "Explicitly record that you have nothing to say this turn, with a brief reason. Silence is the default, so call this only if you want the reason captured for observability. Do not call this in the same turn as a msg or me tool.",
...
func (toolTestAPI) GeneratePersona(context.Context, domain.ModelID, api.PersonaRequest) (string, error) {
