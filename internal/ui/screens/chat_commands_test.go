...
func (stubAPI) GeneratePersonas(context.Context, domain.ModelID) ([]domain.PersonaTemplate, error) {
	return nil, nil
}
...
func TestChatScreen_Commands_specs_are_complete(t *testing.T) {
	...
	require.Equal(t, []spec{
		...
		{Name: "close", Help: "Close the current window, parting the channel if it is one."},
